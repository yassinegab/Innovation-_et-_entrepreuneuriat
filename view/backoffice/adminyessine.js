...
document.addEventListener("DOMContentLoaded", function () {
    ...
    addArticleBtn.addEventListener("click", function () {
      ...
      saveBtn.addEventListener("click", async function (e) {
        ...
        try {
          ...
          // Envoi de la requête
          const response = await fetch("adminyessine.php", {
            method: "POST",
            body: formData,
          });
    ...
    articlesTable.addEventListener("click", function (e) {
      ...
      const saveHandler = async (ev) => {
        ...
        try {
          ...
          <a href="adminyessine.php?id=${cells[0].textContent.trim()}" class="action-icon" title="Supprimer">
            ${originalDeleteHTML}
          </a>
        `;
        } catch (error) {
  ...
  const articleDeleteLinks = document.querySelectorAll(
    "a.action-icon[href^='adminyessine.php?id=']"
  );
