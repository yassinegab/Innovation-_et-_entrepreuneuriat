...
class ArticleController
{
    ...
    public function getRelatedArticles($currentArticleId, $category, $limit = 2)
    {
        try {
            $db = config::getConnexion();
            $sql = "SELECT * FROM article 
                WHERE Catégorie = :category 
                AND ID_Article != :current_id
                ORDER BY Date_de_soumission DESC 
                LIMIT :limit";
    ...
    public function getCommentCount($articleId)
    {
        try {
            $db = Config::getConnexion();
            $sql = "SELECT COUNT(*) FROM commentaires WHERE article_id = :article_id";
            $stmt = $db->prepare($sql);
