...
class ReponseController
{
    ...
    public function ajouterReponse($reponse)
    {
        ...
        try {
            ...
            // Si la réponse est ajoutée avec succès, créer une notification
            if ($result) {
                $this->creerNotificationReponse($reponse->getid_consultation(), $reponse->getid_utilisateur());
            }
    ...
    // Créer une notification pour une nouvelle réponse
    private function creerNotificationReponse($id_consultation, $id_admin)
    {
        ...
        try {
            ...
            $query = "SELECT id_utilisateur, titre FROM consultations WHERE id_consultation = :id_consultation";
            $stmt = $db->prepare($query);
            ...
            if ($consultation) {
                ...
                $titre_consultation = $consultation['titre'];

                // Créer le message de notification
                $message = "Vous avez reçu une réponse à votre consultation: " . $titre_consultation;

                // Insérer la notification
                $query = "INSERT INTO notifications (id_utilisateur, id_consultation, message, date_notification, lu, type) 
                          VALUES (:id_utilisateur, :id_consultation, :message, NOW(), 0, 'reponse')";
                $stmt = $db->prepare($query);
