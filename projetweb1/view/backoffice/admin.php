...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['confirm_inscription'])) {
    ...
    try {
        $query = $pdo->prepare("UPDATE inscription SET statut = 'Confirmée' WHERE id_inscription = ?");
        $query->execute([$id_inscription]);
        $message = "<div class='success-message'>Inscription confirmée avec succès.</div>";
        ...
        $message = "<div class='error-message'>Erreur lors de la confirmation: " . htmlspecialchars($e->getMessage()) . "</div>";
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['refuse_inscription'])) {
    ...
    try {
        $query = $pdo->prepare("UPDATE inscription SET statut = 'Refusée' WHERE id_inscription = ?");
        $query->execute([$id_inscription]);
        $message = "<div class='success-message'>Inscription refusée.</div>";
        ...
        $message = "<div class='error-message'>Erreur lors du refus: " . htmlspecialchars($e->getMessage()) . "</div>";
    }
}
